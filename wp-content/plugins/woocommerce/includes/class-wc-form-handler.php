		...
		if ( ! empty( $_POST['apply_coupon'] ) && ! empty( $_POST['coupon_code'] ) ) {
			...
			if ( $cart_item ) {
				...
				if ( $product && $product->is_in_stock() && $product->has_enough_stock( $cart_item['quantity'] ) ) {
					...
					$removed_notice  = sprintf( __( '%s đã xóa khỏi giỏ hàng.', 'woocommerce' ), $item_removed_title );
					$removed_notice .= ' <a href="' . esc_url( wc_get_cart_undo_url( $cart_item_key ) ) . '" class="restore-item">' . __( 'Khôi phục?', 'woocommerce' ) . '</a>';
				} else {
					/* Translators: %s Product title. */
